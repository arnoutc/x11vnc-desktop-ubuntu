...
RUN curl -O https://bootstrap.pypa.io/get-pip.py && \
    ...
    pip3 install --trusted-host pypi.org --trusted-host files.pythonhosted.org --trusted-host codeload.github.com -U https://github.com/novnc/websockify/archive/refs/tags/v0.10.0.tar.gz && \
    mkdir /usr/local/noVNC && \
    curl -k -s -L https://github.com/x11vnc/noVNC/archive/refs/heads/x11vnc.zip | \
...
RUN apt-get update && \
    ...
    curl -k -s -L https://github.com/LibVNC/x11vnc/archive/refs/heads/master.zip | \
...
RUN groupmod -g $DOCKER_GID $DOCKER_GROUP && \
    usermod -d $DOCKER_HOME -m -u $DOCKER_UID -g $DOCKER_GID -s $DOCKER_SHELL -G sudo $DOCKER_USER && \
